...
class QiDian:  # 爬取每一页的安居客房价信息（爬取海口的房价信息）
    def __init__(self):
        self.msg = []
        self.open_net()
        self.scratch()
    ...
    def scratch(self):
        ...
        div = Html.find('div',{'id':'threadlist'})
        tbodys = div.find_all('tbody')
        #返回tbody列表
        for each in range(1, len(tbodys)):
            try:
                way = tbodys[each].find('em').contents[1].text
                title = tbodys[each].find('a',{'class':'s xst'}).text
                author = tbodys[each].find('a',{'c':'1'}).text
                date = tbodys[each].find('span').text
                reply = tbodys[each].find_all('a',{'c':'1'})[1].text
                self.msg.append([way,title,author,date,reply])
                print(way,title,author,date,reply)
            except:
                continue
...
if __name__ == "__main__":
    ...
    #创建IO流，打开文档，这里先不打开
    workbook2 = xlsxwriter.Workbook('海南大学起点招聘信息.xlsx')  # 创建Excel文件
    worksheet2 = workbook2.add_worksheet()

    data_list = ["招聘类型", "招聘主题", "发帖人", "发帖时间", "最后回复人"]
    row, col = 0, 0
    ...
    url1 = 'http://www.ihain.cn/forum-255-'
    url2 = '.html'
    for i in range(1, 51):
        Message = []
