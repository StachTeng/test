...
class Tieba:        # 爬取每一页的贴吧帖子信息
    ...
    def scratch(self):        
        tieziliebiao=soup.findAll('table', {'class': 'topics-component hide-mobile-table'})
        print(tieziliebiao)
        
        for each in tieziliebiao.findAll('tr',{'class': 'topics-table-row thread','data-action':'topic-item'}):

            try:
                name=each.find('class'=='topic-title-link').text                                    #帖子的主题
                print('haha1') 
                content=each.find('class'=='topic-summary ').text   #帖子的内容    
                print('haha2')
                '''
...
if __name__=="__main__":
    ...
    url1="https://discussionschinese.apple.com/search?q=iphone&page="
    url2="&perPage=60&content=filterDiscussions"

    # print(Proxy_list)

    header_list=['Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/49.0.2623.221 Safari/537.36 SE 2.X MetaSr 1.0']
    header_list.append('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.61 Safari/537.36 Edg/94.0.992.31')
    header_list.append('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.82 Safari/537.36')
    ...
    for i in range(1,5):                                             
        ...
        diaoyong="182.18.174.196:80"                          # 随机调用代理 IP
    ...
    workbook=xlsxwriter.Workbook('社区产品问题.xlsx')                   # 将爬取的信息存入 '海南大学贴吧.xlsx'                                              #创建Excel文件
    worksheet=workbook.add_worksheet()

    data_list=["帖子的主题","帖子的内容"]
    row,col=0,0
